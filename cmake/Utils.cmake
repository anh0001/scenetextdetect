...
# Usage:
#   deepstd_list_unique(<list_variable> [<list_variable>] [...])
macro(deepstd_list_unique)
  foreach(__lst ${ARGN})
    if(${__lst})
...
# Usage:
#   deepstd_clear_vars(<variables_list>)
macro(deepstd_clear_vars)
  foreach(_var ${ARGN})
    unset(${_var})
  endforeach()
endmacro()
...
# Usage:
#   deepstd_string_unique(<string_variable>)
function(deepstd_string_unique __string)
  if(${__string})
    set(__list ${${__string}})
...
# Usage:
#   deepstd_print_list(<list>)
function(deepstd_print_list)
  foreach(e ${ARGN})
    message(STATUS ${e})
  endforeach()
endfunction()
...
# Usage:
#   deepstd_merge_flag_lists(out_variable <list1> [<list2>] [<list3>] ...)
function(deepstd_merge_flag_lists out_var)
  set(__result "")
  foreach(__list ${ARGN})
...
# Usage:
#   deepstd_convert_absolute_paths(<list_variable>)
function(deepstd_convert_absolute_paths variable)
  set(__dlist "")
...
# Usage:
#   deepstd_parse_header(<file> <define1> <define2> <define3> ..)
macro(deepstd_parse_header FILENAME FILE_VAR)
  set(vars_regex "")
...
# Usage:
#   deepstd_parse_header_single_define(<library_name> <file> <define_name>)
function(deepstd_parse_header_single_define LIBNAME HDR_PATH VARNAME)
  set(${LIBNAME}_H "")
  if(EXISTS "${HDR_PATH}")
...
# Usage:
#   deepstd_option(<option_variable> "doc string" <initial value or boolean expression> [IF <condition>])
function(deepstd_option variable description value)
  set(__value ${value})
  set(__condition "")
...
# Usage:
#   deepstd_compare_lists(<list_variable> <list2_variable> [description])
function(deepstd_compare_lists list1 list2 desc)
  set(__list1 ${${list1}})
  set(__list2 ${${list2}})
  list(SORT __list1)
  list(SORT __list2)
...
# Usage:
#   deepstd_warnings_disable(<CMAKE_[C|CXX]_FLAGS[_CONFIGURATION]> -Wshadow /wd4996 ..,)
macro(deepstd_warnings_disable)
  set(_flag_vars "")
  set(_msvc_warnings "")
  set(_gxx_warnings "")
  ...
  deepstd_clear_vars(_flag_vars _msvc_warnings _gxx_warnings)
endmacro()

################################################################################################
# Helper function get current definitions
# Usage:
#   deepstd_get_current_definitions(<definitions_variable>)
function(deepstd_get_current_definitions definitions_var)
...
# Usage:
#   deepstd_get_current_cflags(<cflagslist_variable>)
function(deepstd_get_current_cflags cflags_var)
  get_property(current_includes DIRECTORY PROPERTY INCLUDE_DIRECTORIES)
  deepstd_convert_absolute_paths(current_includes)
  deepstd_get_current_definitions(cflags)

  foreach(i ${current_includes})
    list(APPEND cflags "-I${i}")
  endforeach()

  deepstd_list_unique(cflags)
...
# Usage:
#   deepstd_parse_linker_libs(<deepstd_LINKER_LIBS_var> <directories_var> <libflags_var> <frameworks_var>)
function(deepstd_parse_linker_libs deepstd_LINKER_LIBS_variable folders_var flags_var frameworks_var)
  ...
  foreach(list_elem ${${deepstd_LINKER_LIBS_variable}})
  ...
  deepstd_list_unique(libflags folders)
...
# Usage:
#   deepstd_detect_darwin_version(<version_variable>)
function(deepstd_detect_darwin_version output_var)
  if(APPLE)
